        ...
        if self.has_inner:
            x = x + self.drop_path(
                self.inner_attn(self.inner_norm1(x.reshape(B, N, -1)).reshape(B * N, H_in * W_in, -1), H_in,
                                W_in))  # B*N, k*k, c
            x = x + self.drop_path(
                self.inner_mlp(self.inner_norm2(x.reshape(B, N, -1)).reshape(B * N, H_in * W_in, -1)))  # B*N, k*k, c
...
class PyramidTNT(nn.Module):
    ...
    def __init__(self, configs=None, img_size=252, in_chans=3, num_classes=1000, mlp_ratio=4., qkv_bias=False,
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            out_0 = outer_tokens.reshape(B, H_out, W_out, -1).permute(0, 3, 1, 2).contiguous()
            if i == 0:
                out_1 = self.upsample_2(out_0)
                out_2 = self.bn1(out_1)
                out_3 = self.gelu(out_2)
                outs.append(out_3)
            elif i == 1:
                out_1 = self.upsample_2(out_0)
                out_2 = self.bn2(out_1)
                out_3 = self.gelu(out_2)
                outs.append(out_3)
            elif i == 2:
                out_1 = self.upsample_2(out_0)
                out_2 = self.bn3(out_1)
                out_3 = self.gelu(out_2)
                outs.append(out_3)
            elif i == 3:
                out_1 = self.upsample_2(out_0)
                out_2 = self.bn4(out_1)
                out_3 = self.gelu(out_2)
                outs.append(out_3)
            # print(H_out, W_out)
        ...
        #     # print(outs[i].shape)
        #     if i == 0:
        #         outs[i] = outs[i].reshape(-1, 32, 32, 128).permute(0, 3, 1, 2)
        #     if i == 1:
        #         outs[i] = outs[i].reshape(-1, 16, 16, 256).permute(0, 3, 1, 2)
        #     if i == 2:
        #         outs[i] = outs[i].reshape(-1, 8, 8, 512).permute(0, 3, 1, 2)
        #     if i == 3:
        #         outs[i] = outs[i].reshape(-1, 4, 4, 512).permute(0, 3, 1, 2)
        # print(len(outs))
        # return outer_tokens
        return outs
...
@register_model
def ptnt_s_patch16_256_1(pretrained=False, **kwargs):
    outer_dim = 128
    inner_dim = 8
    outer_head = 4
    inner_head = 2
    configs = {
        'depths': [2, 8, 4, 2],
        'outer_dims': [outer_dim, outer_dim * 2, outer_dim * 4, outer_dim * 8],
        'inner_dims': [inner_dim, inner_dim * 2, inner_dim * 4, inner_dim * 4],
        'outer_heads': [outer_head, outer_head * 2, outer_head * 4, outer_head * 4],
        'inner_heads': [inner_head, inner_head * 2, inner_head * 4, inner_head * 4],
    }
